// Auto-generated. Do not edit!

// (in-package graph_d_exploration.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class MergePointsFeedback {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.percent_completed = null;
    }
    else {
      if (initObj.hasOwnProperty('percent_completed')) {
        this.percent_completed = initObj.percent_completed
      }
      else {
        this.percent_completed = 0.0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type MergePointsFeedback
    // Serialize message field [percent_completed]
    bufferOffset = _serializer.float32(obj.percent_completed, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type MergePointsFeedback
    let len;
    let data = new MergePointsFeedback(null);
    // Deserialize message field [percent_completed]
    data.percent_completed = _deserializer.float32(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 4;
  }

  static datatype() {
    // Returns string type for a message object
    return 'graph_d_exploration/MergePointsFeedback';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '2cfbe45a73922160426e29b7a5f151e0';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    # ====== DO NOT MODIFY! AUTOGENERATED FROM AN ACTION DEFINITION ======
    # Define the action feedback message
    float32 percent_completed
    
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new MergePointsFeedback(null);
    if (msg.percent_completed !== undefined) {
      resolved.percent_completed = msg.percent_completed;
    }
    else {
      resolved.percent_completed = 0.0
    }

    return resolved;
    }
};

module.exports = MergePointsFeedback;
