...
class MergePointsServer:
    ...
    def execute_callback(self, goal):
        ...
        # Print log messages
        rospy.loginfo(f'{self.ServerName_} Server received all points from client')
        result = MergePointsResult()
        result.merged_points, result.radius_used = self.merge_points(goal.points)
        rospy.loginfo(
            f'{self.ServerName_} Merged {len(result.merged_points)} points from all clients')
    ...
    def merge_points(self, list):
        ...
        while(len(unique_points) >= NUMBER_POINTS_LIST_):
            rospy.logwarn(f'Recomputing list: currently {len(unique_points)} points.')
