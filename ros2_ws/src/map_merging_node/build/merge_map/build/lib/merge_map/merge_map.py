...
# Function to compute the amount of squared metres discovered
def compute_discovered_area(map_msg):
    resolution = map_msg.info.resolution

    discovered_cells = sum(1 for cell in map_msg.data if cell == 0 or cell == 1)

    return discovered_cells * (resolution ** 2)
...
def merge_maps(*maps):
    ...
    merged_map.header.frame_id = 'mergedmap'
    ...
    discovered_cells = sum(1 for cell in merged_map.data if cell == 0)

    return merged_map, discovered_cells * (merged_resolution ** 2)
...
class MapMerger(Node):
    def __init__(self):
        ...
        # Publisher
        self.merged_pub = self.create_publisher(OccupancyGrid, '/mergedmap', 10)
    ...
    def map_callback(self, msg, robot_id):
        ...
        if all(self.remove_element(self.maps_received, robot_id)):
            merged_map, disocovered_area = merge_maps(*self.maps)
            #disocovered_area = compute_discovered_area(merge_maps)
            self.get_logger().info(f"Discovered map: {disocovered_area} m2" )
