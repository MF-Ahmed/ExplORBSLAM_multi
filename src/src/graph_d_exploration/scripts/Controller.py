...
def node():
    global frontiers_, merged_frontiers_, mapPoints_, vertices_, edges_, gridMap_data_, is_relocalizing_, robot_, rec_centroids_flag, rec_goal_flag, goal_coordinates, arrAck, status, server_status, markerArray_pu,marker_pub
    ...
    while not rospy.is_shutdown():
        ...
        # Print log
        rospy.loginfo(f'{robot_name}: is waiting for server')
            ...
            for ip in range(0, n_centroids):
                ...
                if n_frontier > 0:
                    ...
                    if log_determinant == -np.inf:
                        rospy.loginfo(rospy.get_name() + f" Log determinant is {log_determinant} applying regularization!")
        ...
        if robot_.getState() == 1:
            ...
            if goal_coordinates != []:
                ...
                winner_frontier_marker = goal_marker(namespace, _goal_[0] ,_goal_[1], "sphere",0.4)             
