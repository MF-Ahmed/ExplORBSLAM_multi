...
nav_msgs::OccupancyGrid collabmapData;
...
void mapCollabCallBack(const nav_msgs::OccupancyGrid::ConstPtr& msg){
  collabmapData=*msg;
}
...
int main(int argc, char **argv){
  ...
  std::string map_topic, collab_map_topic, base_frame_topic, map_frame_id;
  ...
  ros::param::param<std::string>(ns+"/collab_map_topic", collab_map_topic, "/map");
  ...
  ros::Subscriber subCollab= nh.subscribe(collab_map_topic, 100 ,mapCollabCallBack);
  ...
  while (ros::ok()){
    ...
    // Check the availability of the collaborative map
    if(collabmapData.data.size() < 1)
      gridMap = mapData;
    else
      gridMap = collabmapData;
    ...
    while (outside_map == true){
      ...
      if (xr<gridMap.info.origin.position.x || xr>(gridMap.info.origin.position.x+gridMap.info.width * gridMap.info.resolution) || \
          yr<gridMap.info.origin.position.y || yr>(gridMap.info.origin.position.y+gridMap.info.height * gridMap.info.resolution)){
        outside_map = true;
      } else{
        outside_map = false;
      }
    ...
    if (x_new[0]>gridMap.info.origin.position.x && x_new[0]<(gridMap.info.origin.position.x+gridMap.info.width * gridMap.info.resolution) && \
        x_new[1]>gridMap.info.origin.position.y && x_new[1]<(gridMap.info.origin.position.y+gridMap.info.height * gridMap.info.resolution)){      // ObstacleFree    1:free     -1:unkown (frontier region)      0:obstacle
      char checking = ObstacleFree(x_nearest,x_new,gridMap);

  	  if (checking == -1){
  	    exploration_goal.header.stamp=ros::Time::now();
      	exploration_goal.header.frame_id=gridMap.header.frame_id;
