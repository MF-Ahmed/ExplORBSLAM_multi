...
void Node::Update () {
    ...
    if (!position.empty() && trackingState == 2) { // OK tracking state
        ...
        if (was_lost_){
            ROS_INFO("[%s] Successful relocalization.", ros::this_node::getName().c_str());
        ...
        switch (trackingState) {
            case 0: ROS_WARN_THROTTLE(1,"[%s] ORB-SLAM failed: No images yet.", ros::this_node::getName().c_str());
                break;
            case 1: ROS_WARN_THROTTLE(1,"[%s] ORB-SLAM failed: Not initialized.", ros::this_node::getName().c_str());
                break;                
            case 3: ROS_ERROR_THROTTLE(1, "[%s] ORB-SLAM failed: Tracking LOST.", ros::this_node::getName().c_str());
                break;
        }

        ROS_WARN_THROTTLE(1,"[%s] Broadcasting odometry TF from %s to %s.", ros::this_node::getName().c_str(), target_frame_id_param_.c_str(), map_frame_id_param_.c_str());
    ...
    if (publish_pointcloud_param_) {
        ...
        Eigen::Affine3d T_rt(Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitZ()));
            if (_robot_namespace_ != ""){
                if((_robot_namespace_ ==  "drone_0") || (_robot_namespace_ ==  "drone_1"))
                {
                    // Get transformation matrix from camera_link to camera_rgb_frame
                    getTfTransformMatrix(T_rt, _robot_namespace_+"/camera_link", _robot_namespace_+"/camera_rgb_frame");

                    // Perform a rotation of 90 (CCW) degrees around the X-axis
                    Eigen::Isometry3d rotation_matrix_x = Eigen::Isometry3d::Identity();
                    rotation_matrix_x.rotate(Eigen::AngleAxisd(M_PI, Eigen::Vector3d::UnitX()));

                    // Perform a rotation of 90(CW) degrees around the Z-axis
                    Eigen::Isometry3d rotation_matrix_z = Eigen::Isometry3d::Identity();
                    rotation_matrix_z.rotate(Eigen::AngleAxisd(-M_PI/2, Eigen::Vector3d::UnitZ()));

                    // Apply the rotations to the original matrix in sequence
                    T_rt = rotation_matrix_z * rotation_matrix_x * T_rt;
                    //T_rt = rotation_matrix_x * T_rt;

                    // Create a translation vector along the z-axis
                    Eigen::Vector3d translation_vector(0.0, 0.0, 1.1);  // Adjust the values as needed

                    // Apply the translation to the rotated matrix
                    T_rt.pretranslate(translation_vector);
                }
                else {
                    // For other robots, get transformation matrix from camera_rgb_frame to target_frame_id_param_
                    getTfTransformMatrix(T_rt, _robot_namespace_+"/camera_rgb_frame", target_frame_id_param_);
                }
            } 

        else{            
            getTfTransformMatrix(T_rt, "camera_rgb_frame", target_frame_id_param_);
        } 
        ...
        //ROS_INFO_STREAM("cam_base_R:\n"<<cam_base_R_);
        cam_base_T_ = T_rt.translation();
        //ROS_INFO_STREAM("cam_base_T:\n"<<cam_base_T_);
    ...
    if((_robot_namespace_ ==  "drone_0") || (_robot_namespace_ ==  "drone_1"))  // 
        {
            ...
            double temp_x = translation.getX();
            translation.setX(translation.getZ());
            translation.setZ(-temp_x);
            ...
            // Swap X and Z orientations (roll and yaw)
            double temp_roll = roll;
            roll = yaw;
            yaw = -temp_roll;

            // Create new quaternion with modified Euler angles
            tf2::Quaternion new_quat;
            new_quat.setRPY(roll, pitch, yaw);

            // Set the new rotation
            tf_transform.setRotation(new_quat);            
    ...
    if((_robot_namespace_ ==  "drone_0") || (_robot_namespace_ ==  "drone_1"))
        {
            ...
            // Swap X and Z axes of the position
            tf2::Vector3 translation = tf_position.getOrigin();
            double x = translation.getZ(); // Store Z component
            translation.setZ(-translation.getX()); // Set Z component to X component
            translation.setX(x); // Set X component to stored Z component
            tf_position.setOrigin(translation);


            // Extract current rotation
            tf2::Quaternion quat = tf_position.getRotation();
            ...
            // Swap X and Z axis rotations
            double temp = roll;
            roll = yaw; // Swap X and Z axes
            yaw = -temp;
            ...
            // Set the new rotation
            tf_position.setRotation(new_quat);                
        }
