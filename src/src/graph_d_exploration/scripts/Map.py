...
class Map:
    ...
    def getGraphAsMarkerArray(self, global_frame: str = _namespace_+"/map", only_nodes: bool = False,
        ...
        for i in range(0, self.nKFs + 1):
            ...
            # TODO Handle culled KFs (that dict key no longer exist). KF culling disabled for now
            if i_dict in self.Tcw:
                vertex_marker = createMarker(_namespace_, mtype="sphere", frame=global_frame, ns="graph_ns", colors=c1,
        ...
        if not only_nodes:
            ...
            for edge in edges:  # parse edges file
                ...
                if i_dict in self.Tcw and j_dict in self.Tcw:
                    edge_marker = createMarker(_namespace_, mtype="lines", frame=global_frame, ns="graph_ns", colors=c2,
