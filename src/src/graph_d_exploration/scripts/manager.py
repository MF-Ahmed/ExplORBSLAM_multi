...
def cncallback1(data, robot_id):
    #rospy.loginfo(f"/mamager in reward call back got robot_id : {robot_id} and matrix :{data}")
    global m, recr
...
def node():
    ...
    while not rospy.is_shutdown():
        ...
        #rospy.loginfo(f" /manager has idx = {idx} and id = {-1}")            
        if idx == -1 and id == -1:
            rospy.loginfo(rospy.get_name() + ' Waiting for points to merge ...')
        else:
            index_robot = -1
            ...
            if idx != -1:
                ...
                rospy.loginfo(rospy.get_name() +  f" robot_{str(idx)} allowed to use the server. It got the priority after waiting")
            else:
                array_allow[id] = True
                index_robot = id
                # Reset the server requested from the agent
                server_requested[id] = False
                rospy.loginfo(rospy.get_name() + f" robot_{str(id)} allowed to use the server. It has the priority")
            ...
            rows = m.shape[0]
            # Print log
            rospy.loginfo(rospy.get_name() +  ' Sending goal')
            # Pass the goal
            ...
            rospy.loginfo(rospy.get_name() + f' Published goal for robot_{index_robot}')
