...
public:
    FrontierDetector() {
        ...
        nh.param("map_topic3", topic3, std::string("/map3")); // New topic parameter
        ...
        sub3_ = nh.subscribe<OccupancyGrid>(topic3, 10, &FrontierDetector::mapCallback3, this); // New subscriber

        marker_pub_ = nh.advertise<Marker>("frontiers", 10);
        iou_map_pub_ = nh.advertise<OccupancyGrid>("iou_map", 10);
        iou_pub_ = nh.advertise<frontier_detector::iou>("iou_info", 10);
...
private:
    ...
    void mapCallback3(const OccupancyGrid::ConstPtr& msg) { // New callback
        map3_ = msg;
        if (map1_ && map2_) {
            processMaps();
        }
    ...
    void processMaps() {
        if (!map1_ || !map2_ || !map3_) return;

        IoUResult result = computeIoU(map1_, map2_, map3_);
        if (result.iou_map) {
            ...
            publishIoUInfo(result.iou_information);
    ...
    IoUResult computeIoU(const OccupancyGrid::ConstPtr& map1, const OccupancyGrid::ConstPtr& map2, const OccupancyGrid::ConstPtr& map3) {
        IoUResult result;
        float resolution = std::max({map1->info.resolution, map2->info.resolution, map3->info.resolution});
        float min_x = std::max({map1->info.origin.position.x, map2->info.origin.position.x, map3->info.origin.position.x});
        float min_y = std::max({map1->info.origin.position.y, map2->info.origin.position.y, map3->info.origin.position.y});
        float max_x = std::min({
            ...
            map3->info.origin.position.x + map3->info.width * map3->info.resolution});
        float max_y = std::min({
            ...
            map3->info.origin.position.y + map3->info.height * map3->info.resolution});
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                int idx3 = mapIndex(map3, wx, wy); // New index for map3
                int idx_iou = x + y * width;

                if (idx1 != -1 && idx2 != -1 && idx3 != -1) {
                    if (map1->data[idx1] == 0 && map2->data[idx2] == 0 && map3->data[idx3] == 0) {
                        result.iou_map->data[idx_iou] = 0; // free
                    } else if (map1->data[idx1] == 100 && map2->data[idx2] == 100 && map3->data[idx3] == 100) {
                        result.iou_map->data[idx_iou] = 100; // occupied in all maps
                        intersection_count++;
                        union_count++;
                    } else if (map1->data[idx1] == 100 || map2->data[idx2] == 100 || map3->data[idx3] == 100) {
                        result.iou_map->data[idx_iou] = 100; // occupied in at least one map
                        union_count++;
                    }
                }
            }
    ...
    ros::Subscriber sub3_; // New subscriber

    ros::Publisher marker_pub_;
    ros::Publisher iou_map_pub_;
    ros::Publisher iou_pub_;
    ros::Publisher iou_score_pub_;
    ros::Publisher intersection_area_pub_;
    ros::Publisher union_area_pub_;

    OccupancyGrid::ConstPtr map1_;
    OccupancyGrid::ConstPtr map2_;
    OccupancyGrid::ConstPtr map3_; // New map
};
