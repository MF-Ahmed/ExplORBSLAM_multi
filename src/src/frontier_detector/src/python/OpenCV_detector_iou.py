...
def node():
    ...
    targets_pub = rospy.Publisher('detected_points_iou', PointStamped, queue_size=10)
    pub = rospy.Publisher('opencv_shapes_iou', Marker, queue_size=10)
    num_opencv_iou = rospy.Publisher('num_opencv_iou', Int32, queue_size=10)
    ...
    points = createMarker(namespace, frame=map_data_.header.frame_id, ns="markers", colors=[1.0, 1.0, 0.0])

    while not rospy.is_shutdown():
        # Check for the merged map availability
        #rospy.loginfo(f"map_topic  = {map_topic}, namespace = {namespace}, map_topic  = {collab_map_topic} ")
        gridMap_ = map_data_
        frontiers = findFrontiers(gridMap_)
        #rospy.loginfo(f"OpenCV detector with gridMap_frame_ID = {gridMap_.header.frame_id}, frontiers = {frontiers}")
        ...
        for i in frontiers:
            ...
            exploration_goal.header.frame_id = gridMap_.header.frame_id
        ...
        points.id += 1
        points.points = pp
        #rospy.loginfo(f"points = {points.header.frame_id}")        

        pub.publish(points)
        num_opencv_iou.publish(len(points.points))
