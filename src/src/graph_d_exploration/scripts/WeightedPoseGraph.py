...
class WeightedPoseGraph:
    def __init__(self, namespace: str, nodes: list = None, edges: list = None, criteria: str = 'd_opt'):
        """
        Constructor
    ...
    def computeAnchoredL(self):
        ...
        if debug:
            rospy.loginfo("/WeightedPoseGraph: Laplacian anchored through index" + format(idx_to_drop))
        C = L.tocoo()
    ...
    def getGraphAsMarkerArray(self, global_frame: str = "map", color: bool = False) -> MarkerArray:
        ...
        for i in range(1, n+1):
            vertex_marker = createMarker(self.namespace, mtype="sphere", frame=global_frame, ns="graph_ns", colors=c1, lifetime=lt,
                                         alpha=a, scale=sc1)
        ...
        # Add edges
        edge_marker = createMarker(self.namespace, mtype="lines", frame=global_frame, ns="graph_ns", colors=c2, lifetime=lt, alpha=a,
                                   scale=sc2)
    ...
    def hallucinateGraph(self, namespace, robot: Robot, orb_map: Map, seen_cells_pct: float, xy_frontier: ndarray,
        ...
        G_frontier = WeightedPoseGraph(namespace)
        ...
        if n_points > 0:
            ...
            #rospy.loginfo("/WeightedPoseGraph : Plan length " + format(n_points+1) + " generated " + format(plan_nodes)
            ...
            if return_path:
                ...
                id_markers = 0
                for i in new_nodes:
                    marker = createMarker(self.namespace, mtype="arrow", frame="map", colors=[0.0, 0.0, 1.0], lifetime=4, alpha=0.9,
                                          scale=0.5)
            ...
            rospy.logerr("/WeightedPoseGraph : No points in plan to frontier at " + format(xy_frontier) +
