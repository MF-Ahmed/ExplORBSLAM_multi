...
OctoMapper::OctoMapper (ros::NodeHandle &node_handle) {
    ...
    // Getting parameter for robot namespace
    node_handle.getParam(name_of_node_+"/namespace",_namespace_);
...
void OctoMapper::Init () {
    ...
    getTfTransformMatrix(T_optical_target_, _namespace_+"/camera_rgb_frame", target_frame_id_param_);
...
void OctoMapper::initializeParams(ros::NodeHandle &nh){
    ...
    nh.param<std::string>(name_of_node_+ "/target_frame_id", target_frame_id_param_, _namespace_+"/base_link");
    nh.param<std::string>(name_of_node_+ "/pointcloud_frame_id", map_frame_id_param_, _namespace_+"/map");
    nh.param<double>(name_of_node_+ "/max_height", max_height_param_, 3.0);
    nh.param<double>(name_of_node_+ "/min_height", min_height_param_, 0.5);
...
void OctoMapper::MapCallback (const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    if (orb_state_ == 1) { // ORB-SLAM is OK
        ...
        for (int i = 0; i < out_pointcloud.points.size(); i++) {
            if(out_pointcloud.points[i].z < max_height_param_ && out_pointcloud.points[i].z > min_height_param_ ) {
