...
{
    cout << endl << "ORBSLAM 2: Saving camera trajectory to " << filename << " ..." << endl;
    if(mSensor==MONOCULAR)
    {
        cerr << "ORBSLAM 2: ERROR: SaveTrajectoryTUM cannot be used for monocular." << endl;
        return;
    }
    ...
    f.close();
    cout << endl << "ORBSLAM 2: trajectory saved!" << endl;
}


void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    cout << endl << "ORBSLAM 2: Saving keyframe trajectory to " << filename << " ..." << endl;
...
void System::SaveTrajectoryKITTI(const string &filename)
{
    ...
    if(mSensor==MONOCULAR)
    {
        cerr << "ORBSLAM 2: ERROR: SaveTrajectoryKITTI cannot be used for monocular." << endl;
        return;
    }
...
bool System::SetCallStackSize (const rlim_t kNewStackSize) {
    ...
    if (operation_result != 0) {
        std::cerr << "ORBSLAM 2: Error getting the call stack struct" << std::endl;
        return false;
    }

    if (kNewStackSize > rlimit.rlim_max) {
        std::cerr << "ORBSLAM 2: Requested call stack size too large" << std::endl;
        return false;
    }
    ...
    if (rlimit.rlim_cur <= kNewStackSize) {
        ...
        if (operation_result != 0) {
            std::cerr << "ORBSLAM 2: Setrlimit returned result: " << operation_result << std::endl;
            return false;
        }
        return true;
    }
...
void System::EnableLocalizationOnly (bool localize_only) {
  ...
  std::cout << "ORBSLAM 2: Enable localization only: " << (localize_only?"true":"false") << std::endl;
}
...
bool System::SaveMap(const string &filename) {
    ...
    if (!out) {
        std::cerr << "ORBSLAM 2: cannot write to map file: " << map_file << std::endl;
        return false;
    }
    ...
    if (!SetCallStackSize(kNewStackSize)) {
        std::cerr << "ORBSLAM 2: Error changing the call stack size; Aborting" << std::endl;
        return false;
    }

    try {
        std::cout << "ORBSLAM 2: saving map file: " << map_file << std::flush;
        boost::archive::binary_oarchive oa(out, boost::archive::no_header);
        oa << mpMap;
        oa << mpKeyFrameDatabase;
        std::cout << "ORBSLAM 2:  ... done" << std::endl;
        out.close();
        ...
        std::cerr << "ORBSLAM 2: Unknown exception" << std::endl;
        SetCallStackSize(kDefaultCallStackSize);
        return false;
    }
...
bool System::LoadMap(const string &filename) {
    ...
    if (!in) {
        cerr << "ORBSLAM 2: Cannot open map file: " << map_file << " , you need create it first!" << std::endl;
        return false;
    }
    ...
    std::cout << "ORBSLAM 2: Loading map file: " << map_file << std::flush;
    ...
    std::cout << "ORBSLAM 2: Map reconstructing" << flush;
